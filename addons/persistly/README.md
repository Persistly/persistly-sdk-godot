...
Use `accountMode: "authRequired"` when your game allows local saves but requires Firebase sign-in before cloud sync. Call `sign_in_with_firebase_token()` or `sign_in_with_provider()` with `provider: "firebase"` to exchange a Firebase ID token for the Persistly account session used by normal save and sync calls.
