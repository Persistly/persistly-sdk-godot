...
- `auth_firebase_usage.gd` shows `authRequired` mode with `sign_in_with_firebase_token`, `sign_in_with_supabase_token`, provider linking, and linked provider listing. Auth0 games use the same shape with `sign_in_with_auth0_token`.

These examples assume Firebase, Supabase, or Auth0 Auth Bridge is already configured for the Persistly project environment.
